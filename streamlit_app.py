"""
XAUUSD Trading Suite — focused live signal dashboard.
...
st.set_page_config(
    page_title="XAUUSD Signal Engine",
    page_icon="🎯",
    layout="wide",
...
with st.sidebar:
    ...
    st.session_state["api_key"] = st.text_input(
        ...
        type="password",
        help="The key is used only at runtime and is not written into the repository.",
    )
    st.session_state["auto_refresh"] = st.toggle("Auto refresh", value=st.session_state["auto_refresh"])
    refresh = st.button("Run Live Analysis", type="primary", use_container_width=True)
    st.caption("Source: Twelve Data • XAU/USD")
    st.caption("No synthetic/demo market prices are used.")

if st.session_state["auto_refresh"]:
    try:
        from streamlit_autorefresh import st_autorefresh
        st_autorefresh(interval=30000, key="xauusd_live_refresh")
        refresh = True
    except Exception:
        pass
...
if refresh or st.session_state["last_result"] is None:
    ...
    for i, (tf, (interval, size)) in enumerate(intervals.items(), start=1):
        try:
            frames[tf] = fetch_time_series("XAU/USD", interval, size, api_key=api_key)
    ...
    try:
        live_price = fetch_latest_price("XAU/USD", api_key=api_key)
    except Exception as exc:
        live_price = None
    ...
    st.session_state["last_error"] = errors
...
if live_price is not None:
    st.markdown(f'<div class="price">XAU/USD LIVE &nbsp; {float(live_price):.3f}</div>', unsafe_allow_html=True)
else:
...
if status in ("BUY", "SELL"):
    ...
    c1.markdown(f'<div class="label">Entry</div><div class="value">{result["entry"]:.3f}</div>', unsafe_allow_html=True)
    c2.markdown(f'<div class="label">Stop Loss</div><div class="value">{result["sl"]:.3f}</div>', unsafe_allow_html=True)
    c3.markdown(f'<div class="label">Take Profit</div><div class="value">{result["tp"]:.3f}</div>', unsafe_allow_html=True)
...
if st.session_state.get("last_error"):
    with st.expander("Live data warnings"):
        for e in st.session_state["last_error"]:
            st.write("•", e)
    ...
    if df is not None and not df.empty:
        try:
            ...
            fig = go.Figure(go.Candlestick(
                x=p["time"], open=p["open"], high=p["high"], low=p["low"], close=p["close"], name="XAU/USD"
            ))
